...
class Model(object):
    ...
    def forward_propagation(self, x):
        ...
        logits = None
        #Hidden Layer 1
        logits=nn.matmul(x,self.param['w1'])+self.param['b1']
        #Activate
        logits=nn.sigmoid(logits)
        #Hidden Layer 2
        logits=nn.matmul(logits,self.param['w2'])+self.param['b2']
        #Activate
        logits=nn.sigmoid(logits)
        #Final Layer
        logits=nn.matmul(logits,self.param['w3'])+self.param['b3']
        #######################################################################
    ...
    def get_loss(self, x, y):
        ...
        loss, probs = nn.softmax_cross_entropy_loss(None,None,logits,y,-1)
        #######################################################################
    ...
    try:
        ...
        for epoch in range(1, MAX_EPOCHS+1):
            ...
            for step, (x, y) in enumerate(DB.train, 1):
                ...
                #######################################################################
                "*** YOUR CODE HERE ***"
                #forward propagate the model and compute the loss,
...
if __name__ == "__main__":
    ...
    print(MAX_EPOCHS)
    print("Hidden layers - ")
    print(hidden_layer_level_1_size)
    print(hidden_layer_level_2_size)
